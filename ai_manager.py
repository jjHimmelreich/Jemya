...
class AIManager:
    ...
    def generate_system_message(self, has_spotify_connection: bool = False, mcp_mode: bool = False) -> str:
        ...
        base_message = (
            ...
            "• GET CURRENT USER: get_current_user() - Get the current user's Spotify ID and display name\n"
            "• READ PLAYLISTS: read_playlist(playlist_id) - Get all tracks from any playlist\n"
            "• LIST ALL PLAYLISTS: list_playlists() - See ALL user playlists (automatic paging, 184+ supported)\n"
            "• LIST MY PLAYLISTS: list_playlists(owner_id=<user_id>) - Server-side filter by owner. ALWAYS use this pattern when user asks for 'my playlists' or 'playlists I created'\n"
            ...
            "═══════════════════════════════════════════════════════════════════════════════\n\n"
            "Standard Workflow:\n"
            "1. When user asks about 'my playlists' or 'playlists I created': call get_current_user() first, then list_playlists(owner_id=<user_id>)\n"
            "2. For all other playlist listing: call list_playlists() to see all playlists\n"
            "3. Filter results in your logic (by name, year, description, etc.)\n"
            "4. Use read_playlist(playlist_id) with EXACT playlist IDs from step 1\n"
